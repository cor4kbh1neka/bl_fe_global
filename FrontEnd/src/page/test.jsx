...
export const HistorybetdekstopPage = () => {
  ...
  const filteredData = combinedData ? combinedData.filter(bet => {
    ...
    return new Date(bet.orderTime).toDateString() === new Date(selectedDate).toDateString();
  }) : [];
              ...
              <div className="datasechistory">
                {currentBet.length === 0 ? (
                  <div className="empty-message">
                    Data belum tersedia, Silahkan melakukan bet terlebih dahulu.
                  </div>
                ) : (
                  <>
                    <table>
                      <tbody>
                        ...
                        {currentBet
                          .filter(bet => {
                            ...
                            return bet.status === selectedStatus;
                          })
                          .filter(bet => {
                            if (selectedMatch === "all match") return true;
                            if (selectedMatch === "single") return bet.sportsType !== "Mix Parlay";
                            return bet.sportsType === "Mix Parlay";
                          })
                          .map((bet, index) => (
                            <tr key={index}>
                              ...
                              <td className="linkbet">
                                <Link to={`/historybetdekstop/${bet.refNo}/${bet.portoPolio}`}>
  ...
  export default HistorybetdekstopPage;
